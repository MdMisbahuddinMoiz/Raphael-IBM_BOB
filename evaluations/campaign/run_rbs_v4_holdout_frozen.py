#!/usr/bin/env python3
"""
RBS-v4 TERMINAL FALSIFICATION HOLDOUT CAMPAIGN — Frozen Instrument
===================================================================
Authorized: SENTINEL terminal authorization + principal methodological ruling
(AMENDMENT-HOLDOUT-LAUNCH-2026-08-07). Fed by AMENDMENT-A-2026-08-06.

This is the experiment that resolves the central strong-prompting claim that the
earlier RBS-v4 ledger classified as NOT_TESTED (no PROMPTED_AGENT arm ran).

Design (frozen; TERMINAL_FALSIFICATION_PREREGISTRATION + TERMINAL_VALIDATION_FREEZE):
  - Families : 5 FRESH architecture-agnostic families (families.py; T1-T12 burned)
  ...
  - Model    : nvidia/llama-3.3-nemotron-super-49b-v1 (AMENDMENT-A-2026-08-06),
                   identical provider path for ALL arms (model parity, verified).
                   gpt-oss directive values NOT adopted (reconciliation on file).
  - Evaluator: architecture-blind evaluate_runconclusion for all arms (fresh ids).
  - Prompt   : FROZEN (PROMPTED_PROMPT_FREEZE.json).
  - Tag      : ALL frozen `instrument_tag = terminal-holdout-frozen`.
...
Outputs:
  - evaluations/campaign/rbs_v4_holdout.jsonl      (append-only row log, resumable)
  - evaluations/campaign/holdout_runs/             (per-run dirs via runner.save())
  - NO report during collection.
"""
import sys, json, os, time, hashlib
from pathlib import Path

_NO_SCRIPTS = os.path.abspath(os.path.dirname(__file__))  # scripts/ if run there
...
RUN_DIR = Path(_REPO_ROOT) / "evaluations" / "campaign" / "holdout_runs"
...
def run_one(config_id, family_id, relative_seed):
    ...
    row = {
        ...
        "model_id": MODEL_ID,
        "provider": "nvidia",
        "prompt_freeze_sha": "4c2f846495ad0ca8427df19ce253f985765991a79cd7a079e590782537865b5a",
...
def run_campaign(max_cells=None):
    ...
    print("=" * 80)
    print("RBS-v4 TERMINAL FROZEN HOLDOUT — COLLECTION-ONLY")
    print("=" * 80)
